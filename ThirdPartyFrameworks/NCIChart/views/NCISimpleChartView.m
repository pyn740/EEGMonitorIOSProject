...
- (void)defaultSetup{
    ...
    _nciIsSmooth = [[NSMutableArray alloc] init];
        _nciLineWidths = @[@0.3];
    _nciLineColors = [NSMutableArray arrayWithArray: @[[UIColor blueColor], [UIColor redColor], [UIColor purpleColor]]];
    ...
    _nciGridTopMargin = 40;
    _nciGridBottomMargin = 30;
    _nciGridLeftMargin = 38;
    _nciGridRightMargin = 15;
    
    self.backgroundColor = [UIColor clearColor];
    self.chartData = [[LoopQueue alloc] init];
}
...
- (void)layoutSelectedPoint{
    if (selectedPointArgument != selectedPointArgument)
        return;
    NSArray *prevPoint;
    for (int i = 0; i < [_chartData queueLength]; i++){
        NSArray *point = (NSArray *)[_chartData currentItemWithNum:i];
        NSArray *currentPoint = (NSArray *)[_chartData currentItemWithNum:i];
...
- (void)addPoint:(double)arg val:(NSArray *)values{
    //NSLog(@"values:%@",values);
    if (![self.chartData enQueue:@[@(arg), values]]) {
        [self.chartData deQueue];
        [self.chartData enQueue:@[@(arg), values]];
    }
}
