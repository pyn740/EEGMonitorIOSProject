...
- (void) initialize
{
    ...
    [[self frontCamera] addObserver:self forKeyPath:@"adjustingFocus" options:flags context:nil];
#endif

	NSError *error;
	AVCaptureDeviceInput *captureInput = [AVCaptureDeviceInput deviceInputWithDevice:[self frontCamera]  error:&error];
	if (!captureInput)
	{
		NSLog(@"Error: %@", error);
		return;
...
-(void) embedPreviewInView: (UIView *) aView {
    ...
    //NSLog(@"%f,%f,%f,%f",aView.frame.origin.x,aView.frame.origin.y,aView.frame.size.width,aView.frame.size.height);
    //NSLog(@"%f,%f,%f,%f",aView.bounds.origin.x,aView.bounds.origin.y,aView.bounds.size.width,aView.bounds.size.height);
...
{
    //AVCaptureDevice *device = [AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeVideo];
    [[self frontCamera] removeObserver:self forKeyPath:@"adjustingFocus"];
